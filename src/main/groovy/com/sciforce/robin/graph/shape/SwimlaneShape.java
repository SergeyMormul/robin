...
public class SwimlaneShape extends BasicShape
{
	...
	protected double getTitleSize(Graphics2DCanvas canvas, CellState state)
	{
		return Math.max(
				0,
				Utils.getFloat(state.getStyle(), Constants.STYLE_STARTSIZE,
						Constants.DEFAULT_STARTSIZE) * canvas.getScale());
	};
                                          ...
                                          CellState state)
	{
				...
				.isTrue(state.getStyle(), Constants.STYLE_HORIZONTAL, true))
		{
			...
			return new Rectangle(state.getX(), state.getY(),
		...
		else
		{
			...
			return new Rectangle(state.getX(), state.getY(), start,
					state.getHeight());
		}
	}
	...
	{
		double start = getTitleSize(canvas, state);
		String fill = Utils.getString(state.getStyle(),
				Constants.STYLE_SWIMLANE_FILLCOLOR, Constants.NONE);
		boolean swimlaneLine = Utils.isTrue(state.getStyle(),
				Constants.STYLE_SWIMLANE_LINE, true);
		double r = 0;
	...
	protected GraphicsCanvas2D configureCanvas(Graphics2DCanvas canvas,
                                               CellState state, GraphicsCanvas2D c)
	{
		c.setShadow(hasShadow(canvas, state));
		c.setStrokeColor(Utils.getString(state.getStyle(),
				Constants.STYLE_STROKECOLOR, Constants.NONE));
		c.setStrokeWidth(Utils.getInt(state.getStyle(),
				Constants.STYLE_STROKEWIDTH, 1));
		c.setDashed(Utils.isTrue(state.getStyle(), Constants.STYLE_DASHED,
				false));

		String fill = Utils.getString(state.getStyle(),
				Constants.STYLE_FILLCOLOR, Constants.NONE);
		String gradient = Utils.getString(state.getStyle(),
				Constants.STYLE_GRADIENTCOLOR, Constants.NONE);

		if (!Constants.NONE.equals(fill)
				&& !Constants.NONE.equals(gradient))
		{
			Rectangle b = getGradientBounds(canvas, state);
			c.setGradient(fill, gradient, b.getX(), b.getY(), b.getWidth(), b
					.getHeight(), Utils.getString(state.getStyle(),
					Constants.STYLE_GRADIENT_DIRECTION,
					Constants.DIRECTION_NORTH), 1, 1);
		}
		else
		{
			c.setFillColor(fill);
	 ...
	 * 
	 */
	protected void paintSwimlane(Graphics2DCanvas canvas, CellState state,
	...
	protected void paintRoundedSwimlane(Graphics2DCanvas canvas,
                                        CellState state, double start, double r, String fill,
	 ...
	 * Paints the swimlane vertex shape.
	 */
	protected void paintSeparator(Graphics2DCanvas canvas, CellState state,
                                  double start, String color)
	{
		...
		if (!Constants.NONE.equals(color))
		{
			...
			if (Utils.isTrue(state.getStyle(), Constants.STYLE_HORIZONTAL,
					true))
			{
				c.moveTo(w, start);
				c.lineTo(w, h);
