...
public class CurveLabelShape implements ITextShape
{
	...
	public Rectangle updateLabelBounds(String label, Map<String, Object> style)
	{
			...
			else
			{
				...
				for (int i = 0; i < label.length();)
				{
					...
					else
					{
						...
						labelGlyph.labelGlyphBounds = new Rectangle(0, 0,
								width, ascent);
						labelSize += width;
					}
		...
		for (int j = 0; j < labelGlyphs.length; j++)
		{
			...
			{
				labelGlyphs[j].drawingBounds = new Rectangle(minX, minY, maxX
						- minX, maxY - minY);
			}

			if (overallLabelBounds == null)
			{
				overallLabelBounds = (Rectangle) labelGlyphs[j].drawingBounds
						.clone();
			}
			else
			{
		...
		if (overallLabelBounds == null)
		{
			...
			Line labelCenter = curve.getCurveParallel(Curve.LABEL_CURVE,
					0.5);
			overallLabelBounds = new Rectangle(labelCenter.getX(),
