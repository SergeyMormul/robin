...
import com.sciforce.robin.graph.view.ConnectionConstraint;
...
public class EdgeHandler extends CellHandler
{
	...
	public void mouseDragged(MouseEvent e)
	{
		if (!e.isConsumed() && first != null)
		{
			...
			else
			{
				...
				{
					Point point = convertPoint(new Point(e.getPoint()),
							gridEnabledEvent);

					if (points == null)
					{
						points = Arrays.asList(new Point[] { point });
					}
			...
			if (graphComponent.isSignificant(dx, dy))
			{
						...
						|| graphComponent.getGraph().isAllowDanglingEdges())
				{
					movePoint(
							state.getCell(),
							index,
							convertPoint(new Point(e.getPoint()),
									gridEnabledEvent));
				}
	 ...
	 */
	protected void movePoint(Object edge, int pointIndex, Point point)
	{
		IGraphModel model = graphComponent.getGraph().getModel();
	...
	protected void moveLabelTo(CellState edgeState, double x, double y)
	{
		...
		if (geometry != null)
		{
			...
			geometry.setOffset(new Point(Math.round((x - pt.getX()) / scale),
