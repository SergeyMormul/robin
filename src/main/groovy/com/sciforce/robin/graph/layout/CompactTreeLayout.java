...
public class CompactTreeLayout extends GraphLayout
{
	 ...
	 * 
	 */
	protected Rectangle horizontalLayout(TreeNode node, double x0, double y0,
                                         Rectangle bounds)
	{
	 ...
	 * 
	 */
	protected Rectangle verticalLayout(TreeNode node, Object parent,
                                       double x0, double y0, Rectangle bounds)
	{
	...
	protected Rectangle apply(TreeNode node, Rectangle bounds)
	{
		...
		if (cell != null && g != null)
		{
			...
			if (bounds == null)
			{
				bounds = new Rectangle(g.getX(), g.getY(), g.getWidth(),
						g.getHeight());
			}
			else
			{
				bounds = new Rectangle(Math.min(bounds.getX(), g.getX()),
	...
	protected void processNodeOutgoing(TreeNode node)
	{
		...
		for (int j = 0; j < sortedCellsArray.length; j++)
		{
			...
			List<Point> newPoints = new ArrayList<Point>(3);
			double x = 0;
			double y = 0;
