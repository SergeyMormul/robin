...
public class Curve
{
	 ...
	 */
	public static Line INVALID_POSITION = new Line(new Point(0, 0),
			new Point(1, 0));
	...
	public Rectangle getBounds()
	{
		...
		return new Rectangle(minXBounds, minYBounds, maxXBounds - minXBounds,
	...
	public Line getCurveParallel(String index, double distance)
	{
				...
				&& curveIntervals != null && distance >= 0.0 && distance <= 1.0)
		{
			...
			double startPointY = firstPointOfSeg.getY() + segVectorY
					* distanceAlongSeg;
			Point endPoint = new Point(segVectorX / segLength, segVectorY
					/ segLength);
	...
	public Point[] getCurveSection(String index, double start, double end)
	{
				...
				&& end >= 0.0 && end <= 1.0)
		{
			...
			double distanceAlongSeg = (start - curveIntervals[lowerLimit])
					/ (curveIntervals[lowerLimit + 1] - curveIntervals[lowerLimit]);
			Point startPoint = new Point(firstPointOfSeg.getX()
					...
					&& end > curveIntervals[lowerLimit - 1])
			{
				...
				Point endPoint = new Point(firstPointOfSeg.getX()
						+ segVectorX * distanceAlongSeg, firstPointOfSeg.getY()
						+ segVectorY * distanceAlongSeg);
				result.add(endPoint);
			}
	...
	public boolean intersectsRect(java.awt.Rectangle rect)
	{
		...
		{
			Rectangle mxRect = new Rectangle(rect);
	 ...
	 * null is returned.
	 */
	protected Point intersectRectPerimeterPoint(String curveIndex,
												Rectangle rect, int indexSeg)
	{
		Point result = null;
	...
	public Rectangle getRelativeFromAbsPoint(Point absPoint, String index)
	{
		...
		return new Rectangle(distAlongEdge, orthogonalOffset, cartOffsetX,
				cartOffsetY);
	}
	...
	protected void createCoreCurve()
	{
		...
		for (double t = 0; t <= 1.5; t += interval)
		{
			if (t > 1.0)
			{
				...
				t = 1.0001;
				Point endControlPoint = guidePoints
						.get(guidePoints.size() - 1);
				Point finalPoint = new Point(endControlPoint.getX(),
	...
	protected void createLabelCurve()
	{
		...
		for (int i = 1; i < currentCurve.length; i++)
		{
			...
			if (i == 1)
			{
				...
				Point startPoint = new Point(segEndPoint.getX()
			...
			if (i == currentCurve.length - 1)
			{
				// Special case to work out the very end points at
				// the start of the curve
				Point endPoint = new Point(segStartPoint.getX()
